...
gitnexus wiki --profile default  # Preserve the legacy Wiki topology
gitnexus wiki --profile ieee-1016-sdd --lang chinese  # Generate a Chinese detailed design document
...
## Wiki Document Profiles

`gitnexus wiki` uses an LLM and supports five built-in, versioned document profiles:

| Profile            | Contract                                                                                                                                                                           |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `default`          | Preserves the legacy `overview.md`, module pages, `module_tree.json`, and `index.html`. Module names, H1 headings, slugs, and filenames remain stable English values.              |
| `engineering-wiki` | Produces a 27-page engineering Wiki with localized multi-level navigation, a required evidence-backed overall architecture diagram, and visible repository-relative source tables. |
| `arc42`            | Produces independently worded architecture documentation around 12 public arc42 themes.                                                                                            |
| `ieee-1016-sdd`    | Produces a detailed software design description inspired by public IEEE 1016-2009 concepts.                                                                                        |
| `iso-42010-ad`     | Produces an architecture description aligned with public ISO/IEC/IEEE 42010:2022 concepts.                                                                                         |

For non-default profiles, use `--lang chinese` or `--lang zh-CN` to localize deterministic section titles, navigation, fixed text, status labels, and coverage reports. Profile and section IDs, slugs, filenames, evidence IDs, JSON enum values, and official standard names remain stable English values. Unsupported display languages fall back to English and record that fallback in `meta.json` and `coverage.json`; the requested language still guides generated prose.

Each complete generation lives under `.gitnexus/wiki/.generations/<generation-id>/`; `.gitnexus/wiki/.state/current.json` atomically selects the current manifest. Root-level files are compatibility mirrors, and `index.html` reads the selected generation. Non-default profiles additionally include `document_plan.json`, `manifest.json`, `coverage.json`, `coverage.md`, per-section pages, and an aggregate document. `--force` removes reviewed `module_tree.json` / `document_plan.json` state before a full regeneration.

Profile coverage validates required sections, evidence references, and explicit unknown states; it does not prove semantic entailment. **Profile coverage checks do not establish standards conformance.** Built-in standard profiles use independent wording and do not reproduce standards text.
