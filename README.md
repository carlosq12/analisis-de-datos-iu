...
Each `gitnexus analyze` stores the index in `.gitnexus/` inside the repo by default (portable, gitignored) and registers a pointer in `~/.gitnexus/registry.json`; `GITNEXUS_STORAGE_PATH` can redirect one index to an absolute external directory. When an AI agent starts, the MCP server reads the registry and can serve any indexed repo. LadybugDB connections are opened lazily on first query and evicted after 5 minutes of inactivity (max 5 concurrent). If only one repo is indexed, the `repo` parameter is optional on all tools — agents don't need to change anything.
